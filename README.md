...
- 完善 UART 通信协议的可靠性设计，例如增加校验字段、帧计数或异常恢复机制，进一步提升通信稳定性
- 将当前手写 LCD 绘制逻辑逐步抽象为更清晰的界面层，后续可评估引入轻量级 GUI 框架或界面资源生成方案，提高界面设计效率和可维护性
- 匹配不同车辆的转速灯显示，以及仪表盘的UI布局，提高驾驶时的沉浸感
- 增加按键交互与多页面切换能力，根据驾驶状态在主界面、轮胎信息页、策略信息页等不同显示布局之间切换，也可以加入多媒体功能
- 评估使用带宽和扩展性更强的通信接口替代 UART，例如 USB 等方案，以支持更高频率、更大数据量的遥测传输
